...
impl VoteRegistrationJob {
    pub fn start(&self, request: Request) -> Result<JobOutputInfo, Error> {
        ...
        let stake_cert = Path::new(&self.working_dir).join("stake.cert");
        self.cardano_cli
            .stake_address()
            .register_certificate(&stake_vkey_path, &stake_cert)?;

        println!("building transaction..");
        let tx_raw = Path::new(&self.working_dir).join("tx.raw");
        ...
        let tx_signed = Path::new(&self.working_dir).join("tx.signed");
